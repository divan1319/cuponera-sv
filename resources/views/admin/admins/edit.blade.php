...
<div class="mb-8">
    ...
    <h1 class="mt-4 text-3xl font-bold tracking-tight text-gray-900">Editar administrador</h1>
    <p class="mt-2 text-sm text-gray-500">{{ $admin->name }} · {{ $admin->email }}</p>
</div>

<form method="post" action="{{ route('admin.admins.update', $admin->id) }}" class="max-w-2xl space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input id="password" name="password" type="password"
               class="mt-1 w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
        <p class="mt-1 text-xs text-gray-500">Déjala en blanco para mantener la contraseña actual.</p>
